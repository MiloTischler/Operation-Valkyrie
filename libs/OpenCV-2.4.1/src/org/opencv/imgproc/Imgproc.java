 ...
 * @param L2gradient Flag indicating whether a more accurate <em>L_2</em> norm
 * <em>=sqrt((dI/dx)^2 + (dI/dy)^2)</em> should be used to compute the image
 * gradient magnitude (<code>L2gradient=true</code>), or a faster default
 * <em>L_1</em> norm <em>=|dI/dx|+|dI/dy|</em> is enough (<code>L2gradient=false</code>).
 *
...
/**
 * <p>Smoothes an image using a Gaussian filter.</p>
 *
 * <p>The function convolves the source image with the specified Gaussian kernel.
 * In-place filtering is supported.</p>
 *
 * @param src Source image. The image can have any number of channels, which are
 * processed independently. The depth should be <code>CV_8U</code>,
 ...
 * <code>ksize.height</code>, respectively. See "getGaussianKernel" for details.
 * To fully control the result regardless of possible future modifications of
 * all this semantics, it is recommended to specify all of <code>ksize</code>,
 * <code>sigmaX</code>, and <code>sigmaY</code>.
 ...
 * find the correct radius using an additional procedure.</p>
 *
 ...
 * returned that get enough votes (<em>>threshold</em>).
 ...
 * detection, described in [Matas00]. See the line detection example below:</p>
 *
 ...
 * @param dst Destination image of the same size and the same number of channels
 * as <code>src</code>.
 * @param ddepth Destination image depth. See "Sobel" for the list of supported
 ...
 *
 * <p>In all cases except one, the <em>ksize x
 * ksize</em> separable kernel is used to calculate the derivative. When
 * <em>ksize = 1</em>, the <em>3 x 1</em> or <em>1 x 3</em> kernel is used (that
 * is, no Gaussian smoothing is done). <code>ksize = 1</code> can only be used
 * for the first or the second x- or y- derivatives.</p>
 *
 ...
 *
 * <p>See http://code.opencv.org/svn/opencv/trunk/opencv/samples/cpp/contours.cpp
...
/**
 * <p>Smoothes an image using the normalized box filter.</p>
 *
 ...
 * @param anchor Anchor point. The default value <code>Point(-1,-1)</code> means
...
/**
 * <p>Smoothes an image using the box filter.</p>
 *
 * <p>The function smoothes an image using the kernel:</p>
 *
 ...
 * from the corresponding input arrays at the same location. The sample below
 * shows how to compute a 2D Hue-Saturation histogram for a color image.</p>
 *
 * @param images Source arrays. They all should have the same depth,
 * <code>CV_8U</code> or <code>CV_32F</code>, and the same size. Each of them
 * can have an arbitrary number of channels.
 ...
 * <code>CvConvexityDetect</code> is defined as:</p>
 *
 ...
 * <code>src</code> itself but simply constructs the border, for example:</p>
 *
 * <p>Note:</p>
 *
 ...
 *
 *
 * <p><em>S <- fork ((V_(max) - V_(min))/(V_(max) + V_(min)))(if L < 0.5)
 * ((V_(max) - V_(min))/(2 - (V_(max) + V_(min))))(if L >= 0.5)</em></p>
 *
 *
 *
 * <p><em>H <- forkthree ((60(G - B))/(S))(if V_(max)=R)
 * ((120+60(B - R))/(S))(if V_(max)=G)
 * ((240+60(R - G))/(S))(if V_(max)=B)</em></p>
 *
 ...
 * </ul>
 *
 * @param src Source image: 8-bit unsigned, 16-bit unsigned (<code>CV_16UC...</code>),
 * or single-precision floating-point.
 * @param dst Destination image of the same size and depth as <code>src</code>.
 ...
 * will be the same as <code>src.depth()</code>. The following combination of
 * <code>src.depth()</code> and <code>ddepth</code> are supported:
 * <ul>
 *   <li> <code>src.depth()</code> = <code>CV_8U</code>, <code>ddepth</code> =
 * -1/<code>CV_16S</code>/<code>CV_32F</code>/<code>CV_64F</code>
 ...
 * directory.</p>
 *
 * <p>Note: Source <code>image</code> is modified by this function.</p>
 *
...
/**
 * <p>Smoothes an image using the median filter.</p>
 *
 ...
 *
 * @param src Source 1-, 3-, or 4-channel image. When <code>ksize</code> is 3 or
 ...
 * <em>D_(xy)</em> is the mixed derivative.</p>
 *
 ...
 * or up to the specified size.
 * Note that the initial <code>dst</code> type or size are not taken into
 * account. Instead, the size and type are derived from the <code>src</code>,<code>dsize</code>,<code>fx</code>,
 * and <code>fy</code>. If you want to resize <code>src</code> so that it fits
 * the pre-created <code>dst</code>, you may call the function as follows:</p>
 *
 ...
 * CV_INTER_CUBIC (slow) or CV_INTER_LINEAR (faster but still looks OK).</p>
 *
 * @param src Source image.
 * @param dst Destination image. It has the size <code>dsize</code> (when it is
 ...
 * function performs a reverse transformation to "projectPoints". In case of a
 * 3D object, it does not reconstruct its 3D coordinates, but for a planar
 * object, it does, up to a translation vector, if the proper <code>R</code> is
 * specified.</p>
 *
 ...
 *
 * @param src Source image.
 * @param dst Destination image that has the size <code>dsize</code> and the
 * same type as <code>src</code>.
 ...
 *
 * <p><em>dst(x,y) = src((M_11 x + M_12 y + M_13)/(M_(31) x + M_32 y + M_33),
 * (M_21 x + M_22 y + M_23)/(M_(31) x + M_32 y + M_33))</em></p>
 *
 * <p>when the flag <code>WARP_INVERSE_MAP</code> is set. Otherwise, the
