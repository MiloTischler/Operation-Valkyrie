...
public class MultiDirectionSlidingDrawer extends ViewGroup {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (mTracking || mAnimating) {
			...
			if (cache != null) {
				...
				canvas.save();
				if (mInvert) {
					canvas.translate(isVertical ? 0 : handle.getLeft() - mTopOffset - mContent.getMeasuredWidth(),
							isVertical ? handle.getTop() - mTopOffset - mContent.getMeasuredHeight() : 0);
				} else {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mVertical) {
			...
			if (mInvert) {
				...
				content.layout(0, mTopOffset + handleHeight, content.getMeasuredWidth(), mTopOffset + handleHeight
			...
			if (mInvert) {
				...
				content.layout(mTopOffset + handleWidth, 0, mTopOffset + handleWidth + content.getMeasuredWidth(),
						content.getMeasuredHeight());
			}
		}
			...
			case MotionEvent.ACTION_CANCEL: {
				...
				if (Math.abs(velocity) < mMaximumTapVelocity) {
					...
					if (mInvert) {
						...
						c2 = (!mExpanded && handleTop > mBottomOffset + getBottom() - getTop() - mHandleHeight
								- mTapThreshold);
						c3 = (mExpanded && handleLeft < mTapThreshold + mTopOffset);
						c4 = (!mExpanded && handleLeft > mBottomOffset + getRight() - getLeft() - mHandleWidth
								- mTapThreshold);
					}
	...
	private void performFling(int position, float velocity, boolean always) {
		...
		if (mExpanded) {
			...
			Log.d(LOG_TAG, "position: " + position + ", velocity: " + velocity + ", mMaximumMajorVelocity: "
			...
			Log.d(LOG_TAG, "COLLAPSED. position: " + position + ", velocity: " + velocity + ", mMaximumMajorVelocity: "
					+ mMaximumMajorVelocity);
		...
		if (content.isLayoutRequested()) {
			...
			if (mVertical) {
				...
				if (mInvert)
					...
					content.layout(handleWidth + mTopOffset, 0, mTopOffset + handleWidth + content.getMeasuredWidth(),
							content.getMeasuredHeight());
			}
		}
