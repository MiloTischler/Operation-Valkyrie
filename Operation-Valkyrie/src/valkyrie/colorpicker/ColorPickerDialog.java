...
public class ColorPickerDialog extends Dialog {
	...
	private static class ColorPickerView extends View {
		...
		ColorPickerView(Context c, OnColorChangedListener l, int color) {
			super(c);
			mListener = l;
			mColors = new int[] { 0xFFFF0000, 0xFFFF00FF, 0xFF0000FF, 0xFF00FFFF, 0xFF00FF00, 0xFFFF0000 };
			grayColors = new int[] { 0xFFFFFFFF, 0xFF000000, 0xFFFFFFFF };
			Shader s = new SweepGradient(0, 0, mColors, null);
			Shader s2 = new SweepGradient(0, 0, grayColors, null);
			...
			mgrayPaint.setShader(s2);
			mgrayPaint.setStyle(Paint.Style.STROKE);
			mgrayPaint.setStrokeWidth(45);
		...
		protected void onDraw(Canvas canvas) {
			...
			float r2 = CENTER_X - mgrayPaint.getStrokeWidth() * 2.0f;

			canvas.translate(CENTER_X, CENTER_X);

			canvas.drawOval(new RectF(-r, -r, r, r), mPaint);
			canvas.drawOval(new RectF(-r2, -r2, r2, r2), mgrayPaint);
		...
		public boolean onTouchEvent(MotionEvent event) {
			...
			case MotionEvent.ACTION_MOVE:
				if (mTrackingCenter) {
					...
					if (java.lang.Math.sqrt(x * x + y * y) > CENTER_X - mPaint.getStrokeWidth() * 1.0f)
